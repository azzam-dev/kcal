...
(`kcal` اسم المجلد والمستودع؛ الاسم الظاهر `NUTRIVA` نص في قاموس i18n.)

> **الحالة: المرحلة ٠٤ من ١٢ (Onboarding والأهداف) قيد التنفيذ.**
> المراحل ٠١–٠٣ مكتملة: الأساس، والمصادقة والملف الشخصي، ومحرك حساب السعرات والـmacros
> (١٣٥ اختباراً يمر). ومن ٠٤ أُنجز جدول `nutrition_targets` واستعلاماته ورسائل المحرك.
> **٢٢ اختبار RLS مكتوبة ولم تُشغَّل ولا مرة** — تحتاج مفتاحاً في `.env.test.local`.
> **لا يوجد تسجيل طعام ولا شاشات onboarding بعد، ولم يُنشر على Vercel بعد.**
...
npx supabase link --project-ref <ref>   # مرة واحدة لكل مجلد عمل
```

**`link` هو الخطوة التي تُنسى.** بدونه يفشل كل ما بعده بـ
`Cannot find project ref. Have you run supabase link?`، ويُتحقَّق من نجاحه بوجود الملف
`supabase/.temp/project-ref`.

**لا تحتاج كلمة مرور القاعدة:** الـCLI يهيّئ دور دخول عبر واجهة الإدارة. لو طُلبت منك
فاتركها فارغة. وإن احتجتها يوماً فتُضبط من Project Settings ← Database ← Reset database password، ولا تُكتب في أي ملف مُتتبَّع.
...
**المطبَّق على القاعدة:** `20260906072941_enable_search_extensions` و`20260906074514_create_profiles`.
أعِد توليد الأنواع بعد كل migration: `npm run db:types`.
